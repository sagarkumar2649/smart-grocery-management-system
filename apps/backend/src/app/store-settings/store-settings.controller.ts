...
export async function getStoreSettings(_req: Request, res: Response): Promise<void> {
  ...
  if (!settings) {
    settings = await StoreSettings.create({
      ...
      storeDescription: 'Your trusted neighbourhood grocery store for fresh produce and daily essentials.',
    });
  }
...
export async function updateStoreSettings(req: Request, res: Response): Promise<void> {
  ...
  if (files?.heroBanner?.[0]) {
    if (settings.heroBanner?.publicId) {
      await cloudinary.uploader.destroy(settings.heroBanner.publicId).catch(() => {});
    }
  ...
  if (files?.storeFront?.[0]) {
    if (settings.storeFront?.publicId) {
      await cloudinary.uploader.destroy(settings.storeFront.publicId).catch(() => {});
    }
