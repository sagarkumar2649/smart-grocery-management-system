      ...
      <div className="grid gap-8 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2 space-y-3">
          {items.map((item) => (
            <div
              key={item.productId}
              className="flex gap-4 rounded-2xl bg-surface p-4 ring-1 ring-gray-100 shadow-sm"
            >
              {/* Image */}
              ...
              <div className="flex flex-1 flex-col justify-between min-w-0">
                ...
                <div className="flex items-center gap-3 mt-2">
                  <div className="flex items-center rounded-xl border border-gray-200">
                    ...
                    <span className="h-8 w-8 flex items-center justify-center text-sm font-semibold text-foreground border-x border-gray-200">
                      {item.quantity}
                    </span>
                    <button
                      type="button"
        ...
        {/* Summary */}
        <div className="rounded-2xl bg-surface p-6 ring-1 ring-gray-100 shadow-sm h-fit sticky top-24">
          <h2 className="text-lg font-bold text-foreground mb-4">Order Summary</h2>
          <dl className="space-y-3 text-sm">
            <div className="flex justify-between">
              <dt className="text-gray-500">Subtotal ({items.length} items)</dt>
              <dd className="font-semibold text-foreground">{formatINR(total)}</dd>
            </div>
            <div className="flex justify-between">
            ...
            <div className="border-t border-gray-100 pt-3 flex justify-between">
              ...
              <dd className="text-base font-bold text-foreground">{formatINR(total)}</dd>
            </div>
          </dl>
          <Link
            to="/store/checkout"
